...
function load(data) {
    let html = "";
    data.forEach(elem => {
        html += `<div class="card m-2" style="width: 25rem; float:left; padding:5px">
        <img class="card-img-top" src="${elem.imageURI}" alt="Card image cap" />
        <div class="card-body">
        <h5 class="card-title" style="height:3.5em;">${elem.title}</h5>
        <p class="card-text cut-text">${elem.content.split('</p>')[0].replace('<p>', "").replace('</p>', "").replace('<strong>', "").replace('</strong>', "").replace('<h4>', "").replace('</h4>', "").replace('<strong>', "").replace('</strong>', "").replace('<strong>', "").replace('</strong>', "")}</p>
